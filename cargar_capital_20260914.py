...
# -*- coding: utf-8 -*-
"""
Cargue de CAPITAL (card 1484 -> 13608 Julian Sanchez) del 2026-09-14.

Extracto: `2026-09-14_transaction_download.csv` (99 filas, 31-jul -> 10-sep, todas card 1484).
...
Capital genera su Orden por HASH, así que la entrada en la lista NO es opcional: si el emisor
...
CSV = "/Users/julianlopez/Downloads/2026-09-14_transaction_download.csv"
CAS, TARJETA, PREFIJO = "13608", "capital", "capital_"
CARD_NORM = "JULIAN SANCHEZ"
NOTA = "cargue Capital 2026-09-14 (extracto 31-jul al 10-sep)"

REV_ESPERADA = "0165b709a65d29500000002f34b3f21"
PREV_CAPITAL_HIST = 115      # filas capital_ en el histórico antes del cargue
ESP_NUEVAS = 7
ESP_USD_NETO = 187.68
ESP_TIPOS = {"Egreso": 5, "Ingreso": 2}
...
def main():
    ...
    # 🧯 tripwires: este script solo carga Capital
    for fn in ("procesar_amex", "procesar_rakuten", "procesar_robinhood", "procesar_usbank",
               "procesar_intuit", "procesar_egresos", "agregar_incentivo_amex"):
        def _boom(*a, _n=fn, **k):
            raise AssertionError(f"{_n} fue llamada — este script solo carga Capital")
        setattr(mod, fn, _boom)

    print(f"MODO: {'🔴 ESCRITURA REAL' if ESCRIBIR else '🟢 DRY-RUN (0 escrituras)'}")
    print(f"CAPITAL_FECHA_DESDE = {mod.CAPITAL_FECHA_DESDE}")
    ...
    chk(f"{PREV_CAPITAL_HIST} filas {PREFIJO} previas",
        int(o0.str.startswith(PREFIJO).sum()) == PREV_CAPITAL_HIST,
    ...
    raw = pd.read_csv(CSV)
    out = mod.procesar_capital(raw.copy(), fecha_desde=mod.CAPITAL_FECHA_DESDE, cobrados=cobrados,
    ...
    chk("Motivo/Casillero/Orden coherentes",
        set(nuevas["Motivo"]) == {"Tarjeta Capital"}
    ...
    banner("4) 🧪 PRUEBA DEL EXTRACTO CORTO (las 115 ya cargadas se reproducen igual, en USD)")
    todo = mod.procesar_capital(raw.copy(), fecha_desde=mod.CAPITAL_FECHA_DESDE, cobrados=set(),
    ...
    print(f"  {PREFIJO} {PREV_CAPITAL_HIST} → "
    ...
    chk(f"{PREFIJO} = {PREV_CAPITAL_HIST + ESP_NUEVAS}",
        int(o1.str.startswith(PREFIJO).sum()) == PREV_CAPITAL_HIST + ESP_NUEVAS,
    ...
    chk("0 Orden de tarjeta duplicados",
        not o1[o1.str.startswith(("amex_", "capital_", "usbank_", "migracionamex_"))].duplicated().any())
    ...
    if not ESCRIBIR:
        ...
        print("  Para escribir: python3 cargar_capital_20260914.py --escribir")
        return
    ...
    chk2(f"{PREFIJO} = {PREV_CAPITAL_HIST + ESP_NUEVAS}",
         int(q.str.startswith(PREFIJO).sum()) == PREV_CAPITAL_HIST + ESP_NUEVAS)
    ...
    backup_l = f"{carpeta}/{PurePosixPath(remote_l).stem}_backup_{ts}_pre_capital.xlsx"
    ...
    out2 = mod.procesar_capital(pd.read_csv(CSV), fecha_desde=mod.CAPITAL_FECHA_DESDE,
    ...
    for f2 in sorted(os.listdir(ONEDRIVE_DIR)):
        if f2.endswith("_Historico_mayoristas.xlsx") and f2 != os.path.basename(destino):
            shutil.move(f"{ONEDRIVE_DIR}/{f2}",
                        f"{ONEDRIVE_ANT}/{f2.replace('.xlsx','')}_PRE_capital.xlsx")
