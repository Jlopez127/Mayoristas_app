...
# -*- coding: utf-8 -*-
"""
Cargue de ROBINHOOD -> 1444 (Maria Moises), 2026-09-15.
...
Robinhood genera su Orden por HASH (sin hora), así que la entrada en la lista NO es opcional: si el emisor
...
CSV = "/Users/julianlopez/Downloads/ae2b21b4-4b5d-4cd0-bf31-bc8139074a6b.csv"
CAS, TARJETA, PREFIJO = "1444", "robinhood", "robinhood_"
CARD_NORM = "MARIA MOISES"
NOTA = "cargue Robinhood 2026-09-15 (extracto 1-ene al 14-sep)"

REV_ESPERADA = "0165b8520f7320000000002f34b3f21"
PREV_HIST = 249              # filas robinhood_ en el histórico antes del cargue
ESP_NUEVAS = 41
ESP_USD_NETO = 6579.12
ESP_TIPOS = {"Egreso": 40, "Ingreso": 1}
...
def main():
    ...
    # 🧯 tripwires: este script solo carga Robinhood
    for fn in ("procesar_amex", "procesar_rakuten", "procesar_capital", "procesar_usbank",
               "procesar_intuit", "procesar_discover", "procesar_egresos", "agregar_incentivo_amex"):
        def _boom(*a, _n=fn, **k):
            raise AssertionError(f"{_n} fue llamada — este script solo carga Robinhood")
        setattr(mod, fn, _boom)

    print(f"MODO: {'🔴 ESCRITURA REAL' if ESCRIBIR else '🟢 DRY-RUN (0 escrituras)'}")
    print(f"CAPITAL_FECHA_DESDE = {mod.ROBINHOOD_FECHA_DESDE}")
    ...
    raw = pd.read_csv(CSV)
    out = mod.procesar_robinhood(raw.copy(), fecha_desde=mod.ROBINHOOD_FECHA_DESDE, cobrados=cobrados,
    ...
    chk("Motivo/Casillero/Orden coherentes",
        set(nuevas["Motivo"]) == {"Tarjeta Robinhood"}
    ...
    banner("4) 🧪 PRUEBA DEL EXTRACTO CORTO (las 249 ya cargadas se reproducen igual, en USD)")
    todo = mod.procesar_robinhood(raw.copy(), fecha_desde=mod.ROBINHOOD_FECHA_DESDE, cobrados=set(),
    ...
    chk("0 Orden de tarjeta duplicados",
        not o1[o1.str.startswith(("amex_", "rakuten_", "robinhood_", "intuit_", "applepay_", "migracionamex_"))].duplicated().any())
    ...
    if not ESCRIBIR:
        ...
        print("  Para escribir: python3 cargar_robinhood_20260915.py --escribir")
        return
    ...
    out2 = mod.procesar_robinhood(pd.read_csv(CSV), fecha_desde=mod.ROBINHOOD_FECHA_DESDE,
    ...
    for f2 in sorted(os.listdir(ONEDRIVE_DIR)):
        if f2.endswith("_Historico_mayoristas.xlsx") and f2 != os.path.basename(destino):
            shutil.move(f"{ONEDRIVE_DIR}/{f2}",
                        f"{ONEDRIVE_ANT}/{f2.replace('.xlsx','')}_PRE_robinhood.xlsx")
