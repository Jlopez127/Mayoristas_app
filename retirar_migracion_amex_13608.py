...
# -*- coding: utf-8 -*-
"""
Retira el cargo de MIGRACIÓN AMEX de 13608 (Julian Sanchez) — COP 1.227.569,61 / USD 377,29.

Decisión del usuario (2026-09-14): esa compra no es de Julian. Era el ÚNICO cargo del lote de
migración Amex que se le había asignado (nota "Julian" en el archivo fuente).
...
CAS = "13608"
ORDEN = "migracionamex_000016617026178304"
MONTO_ACTUAL = 1227569.61
NOTA = ("Movimientos de migracion Amex - BT*GOWHOLESALE MIAMI CV: — RETIRADO 2026-09-14 "
        "(no es de Julian; Monto=0 en vez de borrar: el guard A no deja perder el Orden)")
...
def main():
    ...
    if not ESCRIBIR:
        ...
        print("  Para escribir: python3 retirar_migracion_amex_13608.py --escribir")
        return
    ...
    for f in sorted(os.listdir(ONEDRIVE_DIR)):
        if f.endswith("_Historico_mayoristas.xlsx") and f != os.path.basename(destino):
            shutil.move(f"{ONEDRIVE_DIR}/{f}",
                        f"{ONEDRIVE_ANT}/{f.replace('.xlsx','')}_PRE_retiro_migracion.xlsx")
            print(f"  archivada: {f}")
